  ...
  return (
    ...
    >
      ...
      <Link to={productUrl} aria-label={`Ver detalles de ${product.name}`} className="block overflow-hidden">
        <div className="relative aspect-[3/4] w-full overflow-hidden bg-surface-hover">
          ...
          <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-surface-base/70 to-transparent pb-2.5 pt-6">
            <span className="flex items-center justify-center gap-1 font-sans text-[10.5px] font-medium uppercase tracking-[0.16em] text-luxury-100">
              Ver producto
      ...
      <div className="pt-2">
        ...
        <h3 className="line-clamp-2 font-sans text-[13.5px] font-normal leading-snug tracking-[0.005em] text-neutral-50 sm:text-[14.5px]">
          <Link to={productUrl} className="transition-colors hover:text-luxury-100">
            {product.name}
          </Link>
        </h3>

        {/* Price — one step heavier than name, luxury gold */}
        <div className="mt-1">
          {product.offer_active && product.offer_quantity && product.offer_price != null ? (
            <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0">
              <span className="font-sans text-[13.5px] font-medium text-luxury-300">
                {product.offer_quantity} x {formatCurrency(product.offer_price)}
              </span>
              <span className="font-sans text-[12px] font-normal text-neutral-500 line-through">
        ...
        <button
          ...
          className="mt-2.5 flex min-h-[44px] w-full items-center justify-center gap-2 rounded-lg bg-luxury-400 px-3 font-sans text-[11px] font-medium uppercase tracking-[0.12em] text-surface-base transition-colors hover:bg-luxury-300 active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-luxury-300/70"
        >
